...
import ListPositionSafety from './PositionSafety';
  ...
  const showMyLocation = () => {  // di chuyển map tới vị trí người thay pin
    ...
    const getLogger = async () => {   // Lấy tất cả Logger về
      try {
       ...
       const ListFinishStolen = LoggerData.filter((item,index) => item.stolen === false && item.stolenLines.length > 0 )
       setListFinishStolen(ListFinishStolen)
...
useEffect(() => { 
    ...
    //   client.subscribe("SAWACO/0001/Latitude");
    //   client.subscribe("0001/Longitude");
    // });
...
const calculateDistance = (positionSafety, positionCurrent ) => {
  const latlng1 = L.latLng(positionSafety);
  const latlng2 = L.latLng(positionCurrent);
                                     ...
                                     <Popup>
                                        <div className='div-popup'>
                                          <div className='infor'>
                                            ...
                                            <div className ='inforItem'>
                                                <div className='title'>Thời điểm:</div>
                                                <div className='value'>{item.timeStamp}</div>    
                                            </div>
